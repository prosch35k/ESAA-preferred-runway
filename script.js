...
async function update() {
    ...
    //Vid inga sektorfilter, visa inget
    if (!ESOSfilter && !ESMMfilter && !GGfilter && !OSfilter && !MLfilter && !OKCfilter) {
        document.getElementById("output").innerHTML = "Please select a sector.";
        return;
    }
    ...
    for (const ap of airports) {
        ...
        try {
            ...
            const tooltip = ap.desc ? ` data-desc="${ap.desc}"` : "";

            const icao = ap.icao.padEnd(6, " ");
            const displayDir = Math.round(wind.dir / 10) * 10; //avrundar närmaste 10-tal
            const dir = String(displayDir).padStart(3, "0");
